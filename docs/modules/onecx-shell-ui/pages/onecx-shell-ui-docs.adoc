...
NOTE: The versions listed in the table above represent the maximum supported versions for each major version. Any OneCX library version less than or equal to the listed version (within the same major) is supported. For example, if v5 supports Angular 18, then v5.0.0, v5.1.0, v5.2.0, etc. are all supported with Angular 18.

For further details on version compatibility, refer to the following resources:

* xref:documentation:onecx-portal-ui-libs:onecx-compatibility.adoc#user-service[OneCX Compatibility Documentation]
