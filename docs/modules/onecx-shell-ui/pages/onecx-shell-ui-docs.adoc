...
The `ocxShellVisibleKeyboardFocus` directive visually highlights the currently focused interactive element, making keyboard focus obvious. This is particularly useful for accessibility testing and for users who navigate with the keyboard.

The directive is attached to the shell portal viewport to enable focus visibility across the UI:

.Example
[source,html]
----
<ocx-shell-portal-viewport ocxShellVisibleKeyboardFocus></ocx-shell-portal-viewport>
----

==== Default Styling and Customization

Focus styles are defined in `src/global-styles.scss` for the class `ocx-focus-host`:
...
[NOTE]
====
In some cases, PrimeNG components may override focus styles. You need to add the following Wrapper rule in the mfe or remote component’s global styles.
...
The directive determines the “focus host” using a set of CSS selectors. It merges a configurable parameter with built-in defaults. This lets you add project-specific widgets or containers that should receive the visible focus ring.
...
.Default selectors included by the shell
...
The directive merges your parameter values with the defaults, then uses `Element.closest()` to find the focus host for the current focused element and applies `ocx-focus-host` while in keyboard mode.
