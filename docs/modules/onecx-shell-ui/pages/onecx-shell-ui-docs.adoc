...
==== React - Libs Version Compatibility
...
NOTE: The major React version in the application determines which OneCX library version should be used. While all versions within the major are compatible, it is **recommended to use the lowest possible OneCX library version** that provides the required features and fixes.
