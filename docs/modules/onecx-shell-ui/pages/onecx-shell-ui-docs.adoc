...
The recommended way to check shell version information is through the **About page** in the Shell UI.
It can be accessed by navigating to the route `/onecx-shell/about-shell` in your local application. That means if your portal is accessible at `https://myDomain.com/portal/admin` the URL for the About page would be `https://myDomain.com/portal/onecx-shell/about-shell`.

The page will display version information for all supported versions.
...
The major Angular version in the application determines which OneCX library version should be used. 

The versions listed in the table above represent the **maximum supported versions for each major version**. Any OneCX library version less than or equal to the listed version (within the same major) is compatible. For example, if the application uses Angular 18, any OneCX library version from v5.0.0 up to the maximum supported v5.x.x version can be used. 

NOTE: While all versions within the major are compatible, it is **recommended to use the lowest possible OneCX library version** that provides the required features and fixes.
